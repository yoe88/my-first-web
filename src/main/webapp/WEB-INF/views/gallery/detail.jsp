...
<main class="container" id="gallery-detail">
    <table class="table">
        <tr>
            <td>
                <h4 class="d-inline">${model.title}</h4><a href="${contextPath}/galleries${qs}" class="btn btn-outline-danger float-right mt-1">목록</a>
            </td>
        </tr>
    ...
    <section class="container d-flex justify-content-around mb-3">
        <sec:authorize access="hasRole('ROLE_ADMIN')"> <%--관리자 이상만 비공개 처리 가능--%>
            <input type="button" value="비공개하기" class="btn btn-outline-purple" onclick="disableGallery('${model.gno}');">
        </sec:authorize>
        <c:if test="${username == model.writer}">        <%--글쓴이만--%>
            <a href="${model.gno}/edit" class="btn btn-outline-success">수정하기</a>
        </c:if>
