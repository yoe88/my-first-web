...
<main id="boardDetail">
        ...
        <table class="table ">
            <tr class="row">
                <td class="col-md-12 clearfix">
                    ...
                    <a href="${contextPath}/admin/boards${qs}" class="btn btn-outline-danger float-right mt-1">목록</a>
                </td>
            </tr>
                ...
                <tr>
                    <td colspan="2" class="py-3 d-flex justify-content-around">
                        <sec:authorize access="hasRole('ROLE_ADMIN')"> <%--관리자 이상만 비공개 처리 가능--%>
                            <input type="button" value="${b.pub ? "비공개하기" : "공개하기"}" class="btn btn-purple" onclick="toggleBoardPub('${b.articleNo}',this);">
                        </sec:authorize>
                        <c:if test="${username == b.id}">
                            <a href="${contextPath}/boards/${b.articleNo}/edit" class="btn btn-purple">수정하기</a>
                        </c:if>
    ...
    <div class="container my-3 clearfix action">
        ...
        <a href="${contextPath}/boards/${b.articleNo}/reply" class="btn btn-outline-danger float-right">답글</a>
    </div>
